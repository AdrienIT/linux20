...
openssl req -new -newkey rsa:2048 -days 365 -nodes -x509 -keyout /opt/ssl/server.key -out /opt/ssl/server.crt -subj "/C=FR/ST=Aquitaine/L=Bordeaux/O=Ynov/OU=L33T/CN=node1.tp2.b2"

mv /opt/ssl/server.key /etc/pki/tls/private/node1.tp2.b2.key
chmod 400 /etc/pki/tls/private/node1.tp2.b2.key
chown web:web /etc/pki/tls/private/node1.tp2.b2.key

mv /opt/ssl/server.crt /etc/pki/tls/certs/node1.tp2.b2.crt
